...
\begin{bibunit}
  ...
  Our na\"ive behavioural type system from Section~\ref{sec:XXX} was too
  ...
  de'Liguoro \& Padovani~\cite{XXX} instead propose the \emph{mailbox calculus}
  ...
  several applications from the Savina benchmark suite~\cite{XXX}. Second, due
  to the receive-only nature of mailboxes, it does not suffer from the
  distributed delegation issues described in Section~\ref{XXX}. Third, even
  without considering message ordering, the type system ensures useful
  properties.
