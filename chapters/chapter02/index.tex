...
\begin{bibunit}
  ...
  \todo{Lopez, Nielson, Nielson paper: Enforcing Availability in Failure-Aware Communicating Systems}
  ...
  An \emph{affine} type system forbids contraction, but allows weakening,
  meaning that session endpoints can be discarded. This is unsound, since to do
  so would allow us to write a program where one party does not complete their
  role in the communication protocol, and the other party is blocked
  indefinitely.

  Nevertheless, in the presence of failure, a linear type system is too strong
  as one party may crash.
  %
  Mostrous \& Vasconcelos~\cite{MostrousV18:affine} introduce the notion of
  \emph{explicit cancellation} to denote an endpoint which can no longer be used
  for communication. An explicit cancellation

  \todo{FowlerLMD19}
  \todo{Pruiksma and Pfenning}
