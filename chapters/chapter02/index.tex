...
problems for future work.
  ...
  problems of distributed delegation (Section~\ref{sec:bt:distrib-deleg}) and the
  difficulties of adding types to distribution-friendly paradigms such as the
  actor model. Finally, in Section~\ref{XXX}
...
without using techniques such as channel priorities~\cite{XXX} or imposing a
tree-like topology on processes~\cite{XXX}, it is not possible to rule out
deadlock when considering multiple sessions.

\emph{Multiparty} session types~\cite{XXX} generalise binary session types to
...
Scribble~\cite{XXX} is a language-agnostic protocol description language, based
