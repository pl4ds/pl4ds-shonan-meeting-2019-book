...
\begin{bibunit}
  ...
  Behavioural types go beyond the power of regular type systems by
    classifying the \emph{behaviour} of an application.
%
...
%
Unfortunately, several behavioural typing disciplines which work well in the
concurrent setting pose challenges in the presence of distribution. In this chapter,
we describe the difficulties with distributed delegation, we discuss
behavioural typing disciplines for concurrency models suited to distribution, we
describe how behavioural typing disciplines can support failure handling, and
finally describe open problems for future work.
}
	%% content
...
typechecking.
  ...
  \mypar{Distributed implementations.}
  ...
  deadlock-freedom are undecidable. Deni\'elou \&
  ...
  behave well according to their specifications, then the communication in the
  ...
  the \emph{type pollution} problem, a particular challenge with typed mailboxes
  ...
  types}, a type discipline inspired by multiparty session types. Communication
...
%
  ...
  techniques such as redundant services and consensus protocols; an example is
