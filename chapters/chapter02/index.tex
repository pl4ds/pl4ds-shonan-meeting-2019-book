...
typechecking.
  ...
  $\pi$-calculus~\cite{KobayashiPT99:linear-pi}. The translation relies
  crucially on variant types and continuation-passing style~\cite{SussmanS98:scheme}.  Kobayashi's
  translation was significantly expanded upon by Dardha et
  al.~\cite{DardhaGS17:revisited}, who proved that the translation was type- and
  semantics-preserving, and showed that the techniques explored by the
  translation could support extensions such as subtyping and polymorphism.

  \todo{Maybe we need a bit more technical information / an example of the CPS
  translation in order to make the implementation description a bit clearer.

  HOLE: describing \texttt{?A and !A types}}
  ...
  session type library for Scala. Linearity is checked at runtime. The library
  represents the linear output type $!A$ and input type $?A$ by two abstract type
  constructors \verb+Out[A]+ and \verb+In[A]+ respectively, and provides a
  local implementation using Scala's
  implementation~\cite{HallerPMKKJ:futures} of futures and
  ...
  The \verb+Out[A]+ type is straightforwardly implemented as an actor reference
  \verb+ActorRef[A]+. However, since it is by design not possible to transmit
  the receive capability of an actor (and mirroring the difficulties with
  distributed delegation), there is no direct analogue to the \verb+In[A]+ type
  in the actor setting.
  ...
  dispatcher to retrieve a value when it arises. Since the buffer of a delegated
  ...
  \todo{Summarise multiparty extension: Scalas et al., 2017}
